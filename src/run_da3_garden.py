...
DATA_DIR = "/rhome/rborth/project/Difix3D/data/garden/images_4"
OUT_DIR  = "/rhome/rborth/project/Difix3D/outputs/da3_garden"
MODEL_ID = "depth-anything/DA3-GIANT-1.1"  # supports GS head
N_VIEWS  = 12
PROCESS_RES = 504
...
stride = max(len(paths) // N_VIEWS, 1)
paths = paths[::stride][:N_VIEWS]
...
# Compute global min/max so the colormap is consistent across views
valid = depth_np > 0
...
extrinsics = torch.from_numpy(prediction.extrinsics).unsqueeze(0).to(gs.means)  # (1, V, 4, 4) world2cam
intrinsics = torch.from_numpy(prediction.intrinsics).unsqueeze(0).to(gs.means)  # (1, V, 3, 3)
...
# color: (1, V, 3, H, W), depth_r: (1, V, H, W)
...
for i, p in enumerate(paths):
    ...
    r   = col_np[i]
    # resize d, r to src size if needed
    h, w = src.shape[:2]
    def to_size(a):
